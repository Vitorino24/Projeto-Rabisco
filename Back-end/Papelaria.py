...
def altera_preco():
    ...
    try:
        ...
        print("Erro! Valor do produto deve ser um número!")
        return
    if novo_valor == produto[3]:
        print("O valor informado é igual ao valor anterior!")
        return
    if novo_valor < 0 or novo_valor > 10000:
        print("Erro: O valor é inválido!")
        return
    
    try:
        comandoSQL = f'UPDATE Produto SET preco = {novo_valor} WHERE idProduto = {id_produto}'
        cursorDB = conexaoDB.cursor()
        cursorDB.execute(comandoSQL)
        conexaoDB.commit()
    except mysql.connector.Error as erro:
